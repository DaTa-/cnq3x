...
static void ARB_MultitextureStage( int stage )
{
	...
	if (r_arb_vertex_buffer_object->integer > 0) {
		qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, szVBOData_TexCoords1_Offset, szVBOData_TexCoords1_Size, svarsMT.texcoords);
		qglTexCoordPointer(2, GL_FLOAT, 0, (GLvoid *)(szVBOData_TexCoords1_Offset));
...
void ARB_StageIterator()
{
	...
	if (r_arb_vertex_buffer_object->integer > 0) {
		szVBOData_XYZ_Offset = 0;
		szVBOData_XYZ_Size = tess.numVertexes * sizeof(vec4_t);
		
		szVBOData_TexCoords0_Offset = szVBOData_XYZ_Offset + szVBOData_XYZ_Size;
		szVBOData_TexCoords0_Size = tess.numVertexes * sizeof(vec2_t);
		
		szVBOData_TexCoords1_Offset = szVBOData_TexCoords0_Offset + szVBOData_TexCoords0_Size;
		szVBOData_TexCoords1_Size = szVBOData_TexCoords0_Size; // no need to compute it again
		
		szVBOData_VertColors_Offset = szVBOData_TexCoords1_Offset + szVBOData_TexCoords1_Size;
		szVBOData_VertColors_Size = tess.numVertexes * sizeof(color4ub_t);
		
		szVBOData_TotalSize = szVBOData_VertColors_Offset + szVBOData_VertColors_Size;

		szVBOIndexes_TotalSize = tess.numIndexes * sizeof(glIndex_t);
  
		qglBindBufferARB(GL_ARRAY_BUFFER_ARB, idVBOData);
		qglBufferDataARB(GL_ARRAY_BUFFER_ARB, szVBOData_TotalSize, NULL, GL_DYNAMIC_DRAW);
		
		qglBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, idVBOIndexes);
		qglBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, szVBOIndexes_TotalSize, tess.indexes, GL_DYNAMIC_DRAW);
	}
	...
	if (r_arb_vertex_buffer_object->integer > 0) {
		qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, szVBOData_XYZ_Offset, szVBOData_XYZ_Size, tess.xyz);
		qglVertexPointer(3, GL_FLOAT, 16, (GLvoid *)(szVBOData_XYZ_Offset));
	}
	else {
	...
	for ( int stage = 0; stage < tess.shader->numStages; ++stage ) {
		...
		if (r_arb_vertex_buffer_object->integer > 0) {
			qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, szVBOData_VertColors_Offset, szVBOData_VertColors_Size, tess.svars.colors);
			qglColorPointer(4, GL_UNSIGNED_BYTE, 0, (GLvoid *)(szVBOData_VertColors_Offset));
			
			qglBufferSubDataARB(GL_ARRAY_BUFFER_ARB, szVBOData_TexCoords0_Offset, szVBOData_TexCoords0_Size, tess.svars.texcoords);
			qglTexCoordPointer(2, GL_FLOAT, 0, (GLvoid *)(szVBOData_TexCoords0_Offset));
		}
		else {
